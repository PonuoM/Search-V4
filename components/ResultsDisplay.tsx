                  ...
                  <tbody className="divide-y divide-slate-200 dark:divide-white/10">
                    {relevantHistory.map((item, index) => (
                      <tr key={index} className="hover:bg-slate-200/40 dark:hover:bg-white/5">
                        ...
                        <td className="px-4 py-2 text-right font-semibold text-green-700 dark:text-green-400">{item.price ? item.price.toLocaleString('th-TH', { style: 'currency', currency: 'THB'}) : '-'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
