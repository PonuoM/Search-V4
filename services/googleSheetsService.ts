...
export const fetchAllDataFromSheet = async (spreadsheetId: string): Promise<{ salesData: SalesHistoryRecord[], contextData: Omit<DataContext, 'customerHistory'> }> => {
    ...
    if (salesDataRange.values && salesDataRange.values.length > 1) {
        ...
        const requiredHeaders = ['วันที่ขาย']; // Only 'วันที่ขาย' is strictly required.
        if (!requiredHeaders.every(h => header.includes(h))) {
            throw new Error(`Sheet '${realtimeSalesSheetName}' is missing required header: ${requiredHeaders.join(', ')}`);
        }
        ...
        dataRows.forEach(row => {
            ...
            // Only require date to be valid. Phone is now optional.
            if (date) {
                const phoneRaw = row[colMap['เบอร์โทร']];
                const phone = (phoneRaw !== null && phoneRaw !== undefined && String(phoneRaw).trim() !== '') 
                    ? formatGoogleSheetPhoneNumber(phoneRaw) 
                    : undefined;
...
export const writeAnalysisToSheet = async (
    ...
    const newRow = [
        ...
        `https://ponuom.github.io/Search-V4/`
    ];
